...
public class TestCases {
		...
		private static void start() {
			...
			System.out.println("------------------------------------------------------------------");
			System.out.println("Valiant effort young adventurer! You challenged "+(round-1)+" rounds! Here are your stats from this dungeon:");
		...
		private static int[] spawn(int round) {
			if(round == 1) {
				int monster[] = {10, 10, 3, 2}; //50% chance to win with weapon
				return monster;
			} else if(round == 2) {
				int monster[] = {6, 10, 10, 2}; //win using book
				return monster;
			} else if(round == 3) {
				int monster[] = {10, 10, 10, 1}; //win doing nothing
				return monster;
			} else if(round == 4) {
				int monster[] = {10, 3, 10, 3}; //win by running away
				return monster;
			} else {
				int monster[] = {10, 10, 10, 3}; //cannot win
				return monster;
			}
		}
